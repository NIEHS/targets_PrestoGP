...
WBD <- st_read("/Volumes/SHAG/WBD-National/WBD_National_GDB.gdb",layer = "WBDHU6")
...
# HUC06
HUC.rast.vals <- data.table("huc06" = data.AZO$huc06)

huc06.unique <- unique(data.AZO$huc06)

HUC.rast.vals[,paste0("huc06",".",names(OLM.stack.values)) := NA_real_]

HUC.rast.class <- data.table("huc06" = data.AZO$huc06)
...
HUC.rast.class[,paste0("huc06",".",class.possible.names) := 0]

for (i in 1:length(huc06.unique)){
  print(i)
  
  # get index - for rows of output - where data match the given HUC06
  idx.row <- data.AZO$huc06 == huc06.unique[i]
  
  # get the given HUC06 geometry from the WBD data
  huc06.polygon <- dplyr::filter(WBD,huc6 == huc06.unique[i])
 
  # calculate the mean raster values in the HUC
  huc06.val <- exact_extract(OLM.stack.values,st_geometry(huc06.polygon),fun = "mean", stack_apply = TRUE)

  # Assign the extracted values to the appropriate location in the output
  HUC.rast.vals[idx.row,2:ncol(HUC.rast.vals)] <- huc06.val
  
  # calculate the fraction of each raster class in the HUC
  extract.raster.classes <- exact_extract(OLM.stack.classes,huc06.polygon,fun = "frac", stack_apply = TRUE)
...
data.AZO.HUC06.OLM <- cbind(data.AZO,HUC.rast.vals,HUC.rast.class)
sf::st_write(data.AZO.HUC06.OLM,"/Volumes/SHAG/PrestoGP_Covariates/AZO_HUC06_OLM.gpkg")
