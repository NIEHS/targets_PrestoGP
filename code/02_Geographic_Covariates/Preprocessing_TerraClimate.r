...
path_tc_files <- list.files(path = path_tc, pattern = "*.nc$",
                full.names = TRUE)


# some bands should be summed
bandnames <- c("aet", "def", "PDSI", "pet", "ppt", "q", "soil", "srad", 
...
netcdf_read_sum <- 
    ...
    lapply(function(x) { grep(paste0("(", x, ")"), path_tc_files, value = TRUE)}) |>
    lapply(function(x) { Reduce(c, lapply(x, terra::rast)) |> preprocess(ext_mainland, "sum")}) |>
    Reduce(f = c, x = _)

netcdf_read_mean <- 
    split(bandnames, bandnames) |>
    lapply(function(x) { grep(paste0("(", x, ")"), path_tc_files, value = TRUE)}) |>
    lapply(function(x) { Reduce(c, lapply(x, terra::rast)) |> preprocess(ext_mainland, "mean")}) |>
    Reduce(f = c, x = _)
