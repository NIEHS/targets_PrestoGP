calc_olm_huc <- function(data.AZO, olm_raster, wbd_data, hucunit){
  ...
  if (any(grepl("TextureClass",names(olm_raster)))) {
    ...
    # For each depth level, assign texture classes
    for (i in 1:6) {
...
calc_olm_huc_new <- function(data.AZO, olm_raster, wbd_data, hucunit){
  ...
  # Update hucunit variable -
  # Pad zeros for single digit hucs to be 2 digits (i.e. huc8 -> huc08)
  if (nchar(hucunit) == 4) {
    colnames(WBD)[colnames(WBD) == hucunit] <-
      paste0("huc0", str_sub(hucunit, 4))
    hucunit <- paste0("huc0", str_sub(hucunit, 4))
  }
  if (hucunit %in% c("huc10", "huc12")) {
    WBD$split_level <- str_sub(WBD[[hucunit]], 1, 6)
  }
  ...
  if (any(grepl("TextureClass", names(olm_raster)))) {
    ...
    olm_values <-
      data.table(hucunit = data.AZO[[hucunit]]) |>
      setnames(hucunit)

    class.df <- class.df[order(class.df$Var1), ]
    class.possible.names <-
      paste0("frac_", class.df$Var1, ".", class.df$Var2)

    res <-
      par_hierarchy(
        WBD,
        regions_id = "split_level",
        unit_id = hucunit,
